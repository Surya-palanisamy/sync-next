...
const io = socketIo(server, {
  cors: {
    origin: ["https://sync-next.vercel.app", "http://localhost:3000"],
  ...
  socket.on("authenticate", (userData) => {
    ...
    if (teams && teams.length > 0) {
      teams.forEach((team) => {
        ...
        console.log(
          `🏠 User ${username} joined team room: ${team.name} (${teamId})`
        );
    ...
    console.log(
      `✅ User ${username} authenticated and joined ${teams?.length || 0} teams`
    );
  });
